...
export class GiftapiOrderService {
  ...
  async createOrder(
    ...
    try {
      this.logger.log(
        `Creating GiftAPI order: ${extId} for SKU ${skuId}`,
      );
      ...
      if (product.stock <= 0) {
        throw new BadRequestException(
          `SKU ${skuId} is out of stock`,
        );
      }

      // Создать заказ в GiftAPI
      const response = await this.giftapi.createOrder(
        extId,
        skuId,
        fields,
      );
      ...
      if (!response.success) {
        throw new Error(
          `GiftAPI error: ${response.error || 'Unknown error'}`,
        );
      }
      ...
      this.logger.log(
        `GiftAPI order created successfully: ${giftapiOrder.id}`,
      );
  ...
  async handleWebhook(payload: any) {
    const { event, idempotency_key, data, timestamp } = payload;
  ...
  private async handleOrderStatusChanged(
    ...
    try {
      ...
      const deliveryData =
        giftapiOrder.items?.[0]?.delivery_data || [];
      ...
      if (
        ...
        this.logger.log(
          `Order ${ourOrder.id} final status: ${ourStatus}`,
        );
      }
    } catch (error) {
      this.logger.error(
  ...
  async topupEsim(
    ...
    try {
      this.logger.log(
        `Creating eSIM top-up: ${extId} for ICCID ${iccid}`,
      );

      const response = await this.giftapi.topupEsim(
        iccid,
        extId,
        skuId,
      );
      ...
      this.logger.error(
        `Failed to create eSIM top-up for ${orderId}:`,
        error,
      );
      throw error;
    }
  ...
  async getEsimStatus(iccid: string) {
    try {
      ...
      this.logger.error(
        `Failed to get eSIM status for ${iccid}:`,
        error,
      );
      throw error;
    }
